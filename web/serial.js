...
class SurrealSerialFlasher {
    ...
    async connect() {
        ...
        if (typeof this.port.setSignals === 'function') {
            try {
                ...
                console.debug('Serial control-signal setup skipped:', e);
            }
        }
    ...
    async disconnect() {
        try {
            ...
            if (this.port) {
                // Reset serial control lines before closing.
                if (typeof this.port.setSignals === 'function') {
                    await this.port.setSignals({
    ...
    consumeToken(token) {
        ...
        if (idx !== -1) {
            ...
            while (
                endIdx < this.rxBuffer.length &&
                (
                    this.rxBuffer[endIdx] === '\r' ||
    ...
    async waitForToken(token, timeoutMs = 8000) {
        ...
        while (Date.now() - startTime < timeoutMs) {
            ...
            if (readValue) {
                this.rxBuffer += this.decoder.decode(
                    readValue,
                    { stream: true }
                );
            }
        ...
        throw new Error(
            `Timeout waiting for '${token}'. Buffer: ${this.rxBuffer}`
        );
    }
    ...
    async flashPayload(payloadBytes, progressCallback) {
        ...
        if (progressCallback) {
            progressCallback(
                0,
                'Initiating handshake with Pico...'
            );
        }

        await this.writer.write(
            encoder.encode('SBPT_FLASH\n')
        );
        ...
        if (progressCallback) {
            progressCallback(
                5,
                `Sending payload length (${(payloadBytes.length / 1024).toFixed(1)} KB)...`
            );
        }

        const lenBuf = new Uint8Array(4);

        new DataView(lenBuf.buffer).setUint32(
            0,
            payloadBytes.length,
            true
        );
        ...
        const totalSectors = Math.ceil(
            payloadBytes.length / SECTOR_SIZE
        );
        ...
        for (let i = 0; i < totalSectors; i++) {
            ...
            const pct = Math.round(
                5 + ((i + 1) / totalSectors) * 90
            );

            if (progressCallback) {
                progressCallback(
                    pct,
                    `Flashing to RP2350 (${i + 1}/${totalSectors} sectors — ${pct}%)...`
                );
            }
        }
        ...
        if (progressCallback) {
            progressCallback(
                98,
                'Verifying flash integrity on device...'
            );
        }
        ...
        if (progressCallback) {
            progressCallback(
                100,
                'Flash verified! Pico rebooted and ready.'
            );
        }
